...
class WhenLoop;
class TillLoop;
class LoopStmt;  // loop(start, stop, step) construct
class DeferStmt;
class ForLoop;
class WhileLoop;
class BreakStmt;
...
public:
    ...
    virtual void visit(LoopStmt* node) = 0;  // loop(start, stop, step) construct
    ...
    // Range expressions (Audit task #9)
    virtual void visit(RangeExpr* node) { /* default: do nothing */ }
};
