...
public:
    enum OpType {
        ADD, SUB, MUL, DIV, MOD,
        EQ, NE, LT, GT, LE, GE,
        SPACESHIP,  // <=> three-way comparison (-1, 0, 1)
        ...
        PIPE_FORWARD, PIPE_BACKWARD,  // |> and <| pipeline operators
...
// Range Expression
// Example: 1..10 (inclusive), 0...100 (exclusive)
class RangeExpr : public Expression {
public:
    std::unique_ptr<Expression> start;
    std::unique_ptr<Expression> end;
    bool is_exclusive;  // true for ..., false for ..

    RangeExpr(std::unique_ptr<Expression> s, std::unique_ptr<Expression> e, bool exclusive = false)
        : start(std::move(s)), end(std::move(e)), is_exclusive(exclusive) {}
