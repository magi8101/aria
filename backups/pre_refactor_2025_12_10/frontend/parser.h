...
private:
    ...
    // Recursion depth tracking (Critical Bug Fix: Prevent stack overflow)
    ...
    std::unique_ptr<Expression> parseNullCoalesce();  // Null coalescing: ??
    std::unique_ptr<Expression> parsePipeline();  // Pipeline operators: |> and <|
...
public:
    ...
    std::unique_ptr<Statement> parseLoopStmt();  // loop(start, stop, step) construct
