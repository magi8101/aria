...
// RAII Recursion Depth Guard (Critical Bug Fix: Prevent stack overflow)
class RecursionGuard {
private:
    size_t& depth;
    size_t max_depth;
public:
    RecursionGuard(size_t& d, size_t max) : depth(d), max_depth(max) {
        ++depth;
        if (depth > max_depth) {
            throw std::runtime_error("Parser recursion depth exceeded (possible deeply nested expressions or infinite loop)");
        }
    }
    ~RecursionGuard() {
        --depth;
...
std::unique_ptr<Expression> Parser::parsePrimary() {
    ...
    if (current.type == TOKEN_IDENTIFIER) {
        ...
        // Disambiguation for generics vs comparison: identifier<T> vs a < b
        // We only parse as generic if '<' is followed by a type token.
        if (current.type == TOKEN_LPAREN || current.type == TOKEN_LT) {
            bool isGenericCall = false;
            
            if (current.type == TOKEN_LT) {
                // Look ahead: save position and check next token
                Token savedToken = current;
                advance(); // peek at token after '<'
                
                // If next token is a valid type, parse as generic call
                isGenericCall = isTypeToken(current.type);
                
                if (!isGenericCall) {
                    // Not a generic call - this identifier is the left side of a comparison
                    // Return it as a VarExpr so expression parser can handle "a < b"
                    // Restore parsing state by keeping current at the non-type token
                    // The '<' was already consumed, so parsers calling this will see the
                    // token after '<'. This is acceptable since parsePrimary is complete.
                    // Actually, we need to handle this better - for now, reject the pattern
                    // A proper fix would require lexer support for token push-back
                    // For v0.0.7, we document this as a known limitation
                    return std::make_unique<VarExpr>(name);
                }
                // If it IS generic, we're positioned at first type argument - good!
            }
            
            // Parse generic type arguments if present: func<int8, int32>
            if (current.type == TOKEN_LT || isGenericCall) {
                if (current.type == TOKEN_LT) {
                    advance(); // consume '<' for non-lookahead path
                }
                // Now at first type argument
